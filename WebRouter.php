...
class WebRouter {
	...
	/** @var String */
	private $viewName;				// The version of the API to use
	...
	public function GetParsedPath() {
		return dirname(__FILE__) . '/Views/' . $this->viewName . '/' . $this->viewName . '.php';
	}
	...
	public function Route() {
		...
		if ($count == 0) {
			...
			$this->viewName = $this->requestPath[0];
		}

		// If the specified view name is not found as a file then throw an exception
		if (!file_exists($this->GetParsedPath())) {
			throw new \WebsiteException('The requested view (' . $this->viewName . ') is missing', 800);
		}
	}
	...
	public function Execute() {
		...
		// Render the title from the requested view class
		call_user_func_array(array($handle, 'Title'), array());

		// Close the website title tag
		include_once(__DIR__.'/Templates/endTitle.html');

		// Load in the javascript files  to the header for the requested view class
		call_user_func_array(array($handle, 'JS'), array());

		// Load in the css files to the header for the requested view class
		...
		// Render the main body of the website from the requested view class
		...
		// Show any view specific footer information from the requested view class
		call_user_func_array(array($handle, 'Footer'), array());

		// Load in any javascript to the end of the body from the requested view class
