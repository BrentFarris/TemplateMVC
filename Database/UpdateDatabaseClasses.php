...
function Tables($name) {
	$dbName = \Common\Settings\Settings::GetSetting($name, 'database');
	...
	foreach ($tables as $table) {
		...
		foreach ($fields as $field) {
			...
			if ($default == 'NULL' || !strlen($default)) {
				$default = '';
			...
			if (strpos($field['Type'], 'int(') !== false) {
				$cast = '(int)';
			} else if (strpos($field['Type'], 'float') === 0 || strpos($field['Type'], 'double') === 0 || strpos($field['Type'], 'decimal') === 0) {
				$cast = '(float)';
			}

			$classFields .= 'protected $'  .$fieldName . $default . '; function Get' . (str_replace(' ', '', ucwords(str_replace('_', ' ', $fieldName)))) . '() { return ' . $cast . '$this->' . $fieldName . '; } function Set' . (str_replace(' ', '', ucwords(str_replace('_', ' ', $fieldName)))) . '($val) { $this->' . $fieldName . ' = $val; $this->SetFieldDirty("' . $fieldName . '"); } ';
